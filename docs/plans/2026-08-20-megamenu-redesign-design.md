...
- A **floating "pill" header variant** is the default (toggle to compare). Lower radius (`--radius-m`), and it **stays floating on mobile**. When a mega menu opens the **pill stays a pill (no grow)** and the panel becomes a **detached rounded card matched to the pill width** with a small gap below (`.header--floating .header__megamenu`) — the panel's internal widths switch from `vw` to `%` so they fit the card. The gap means the pill's rounded corners reveal the page, not a panel edge, so there are no coloured corner arcs.
